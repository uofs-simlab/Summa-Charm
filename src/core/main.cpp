...
const std::string command_line_help =
    "Summa-Actors is in active development and some features may not be available.\n"
    "Usage: summa_actors -m master_file [-g startGRU countGRU] [-c config_file]\n"
    "Available options: \n"
    "\t-m, --master:         Define path/name of master file (can be specified in "
    "config)\n"
    "\t-g, --gru:            Run a subset of countGRU GRUs starting from index startGRU "
    "\n"
    "\t-c, --config:         Path name of the Summa-Actors config file (optional but "
    "recommended)\n"
    ...
    "\t-n, --newFile         Define frequency [noNewFiles,newFileEveryOct1] of new "
    "output files\n"
...
public:
  ...
  Main(CkArgMsg* m)
  {
    ...
    {
      if ((strcmp(m->argv[it], "-m") == 0 || strcmp(m->argv[it], "--master") == 0) &&
          it + 1 < m->argc)
      {
        master_file = m->argv[++it];
      }
      else if ((strcmp(m->argv[it], "-g") == 0 || strcmp(m->argv[it], "--gru") == 0) &&
               it + 2 < m->argc)
      {
        startGRU = atoi(m->argv[++it]);
        countGRU = atoi(m->argv[++it]);
      }
      else if ((strcmp(m->argv[it], "-c") == 0 || strcmp(m->argv[it], "--config") == 0) &&
               it + 1 < m->argc)
      {
    ...
    if (master_file != "")
      settings.job_actor_settings_.file_manager_path_ = master_file;
